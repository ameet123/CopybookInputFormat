    ...
    public CopybookRecordReader(FileSplit genericSplit, JobConf job) {
        try {
            ...
            if (cblPath == null) {
                ...
                if (mrwork == null) {
                    System.out.println("When running a client side hive job you have to set \"copybook" +
                            ".inputformat.cbl.hdfs.path\" before executing the query.");
                    System.out.println("When running a MR job we can get this from the hive TBLProperties");
                }
                Map<String, PartitionDesc> map = mrwork != null ? mrwork.getPathToPartitionInfo() : null;
            ...
            BufferedInputStream fileIn = new BufferedInputStream(fs.open(split.getPath()));
